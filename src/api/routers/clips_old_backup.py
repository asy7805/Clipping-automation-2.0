...
async def get_clips(
    ...
    try:
            ...
            try:
                ...
                for item in items:
                    ...
                    if item_name.endswith('.mp4'):
                        ...
                        # No thumbnail generation for MVP
        ...
        for clip in paginated_clips:
            # Generate varied score based on file size (larger files tend to be more interesting)
            # This creates visual variety until we have real AI scores
            file_size = clip.get('size', 0)
            base_score = 0.5 + (min(file_size, 50_000_000) / 100_000_000)  # 0.5 to 1.0 based on size
            # Add some variation based on file name hash
            variation = (hash(clip['file_name']) % 20) / 100  # -0.1 to +0.1
            varied_score = max(0.3, min(0.95, base_score + variation))
            
            # Build clip response with thumbnail
            clip_resp = ClipResponse(
                ...
                confidence_score=round(varied_score, 2),  # Varied score based on file
